...
const Listen = {
  ...
  create: (listen: IListen) => {
    const info = runQuery(
      'INSERT INTO listens (ts, platform, ms_played, track_id, user_id, reason_end) VALUES (?, ?, ?, ?, ?, ?)',
  ...
  update: (id: number, listen: IListen) => {
    const info = runQuery(
      'UPDATE listens SET ts = ?, platform = ?, ms_played = ?, track_id = ?, user_id = ?, reason_end = ? WHERE id = ?',
  ...
  delete: (id: number) => {
    const info = runQuery('DELETE FROM listens WHERE id = ?', [id]);
    return { changes: info.changes };
  },
};
